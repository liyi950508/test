...
<html>
	...
	<head>
		...
		<link rel="stylesheet" href="css/Course/quest_watch.css" />
	</head>
				...
				<div class="lesson-list">
					<div class="lesson-item">
						<div class="v-item">
							<div class="pic">
								<div class="video-pic">
									<a href="videoAction_getVideoByName?videoId=<s:property value=" #st.videoId "/>">
				...
				<s:iterator value="#request.resource" var="st">
					<div class="resource">
						<div class="quest-name">
							<a class="a-cont" href="sheQuAction_getQuestByAnswer?questionId=<s:property value="#st.questionId"/>"><s:property value="#st.questionName" /></a>
						</div>
				...
				<div class="feiye-nav">
					<ul>
						<c:choose>
							...
							<c:otherwise>
								<li>
									<a class="a-cont" href="videoAction_getVideoByQuest?videoId=${video.getVideoId()}&currentPage=1">«首页</a>
								</li>
								<li>
									<a class="a-cont" href="videoAction_getVideoByQuest?videoId=${video.getVideoId()}&currentPage=${pageBean.currentPage-1}">«上一页</a>
								</li>
							</c:otherwise>
						</c:choose>
						...
						<c:forEach begin="${pageBean.forEachBegin }" end="${pageBean.forEachEnd}" varStatus="tp">
							<c:choose>
								...
								<c:otherwise>
									<li>
										<a class="a-cont" href="videoAction_getVideoByQuest?videoId=${video.getVideoId()}&currentPage=${tp.index}">${tp.index}</a>
									</li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						...
						<c:choose>
							<c:when test="${pageBean.currentPage!=pageBean.totalPage}">
								<li>
									<a class="a-cont" href="videoAction_getVideoByQuest?videoId=${video.getVideoId()}&currentPage=${pageBean.currentPage+1}">下一页»</a>
								</li>
								<li>
									<a class="a-cont" href="videoAction_getVideoByQuest?videoId=${video.getVideoId()}&currentPage=${pageBean.totalPage}">尾页»</a>
								</li>
							</c:when>
							<c:otherwise>
								<li>
