...
<html>
	...
	<head>
		...
		<link rel="stylesheet" type="text/css" href="css/personal/Personal.My_Collection.css">
	</head>
				...
				<div class="show">
					...
					<div class="a-a">
						<s:iterator value="#request.resource" var="st">
							<ul class="nav">
								<li id="li-1">
									<p><s:property value="#st.selectName" /><p>
								</li>
								<li id="li-2">
									<p><s:property value="#st.uploadUser" /><p>
								</li>
								<li id="li-2">
									<p><s:property value="#st.label" /><p>
								</li>
								<li id="li-2">
									<p><s:property value="#st.selectTime" /><p>
								</li>
								<li id="li-2">
									<div class="AAA"><a href="userSelectAction_DeleteCollentResource?selectId=<s:property value =" #st.userselectId "/>">取消收藏 </a></div>
								</li>
							</ul>
						</s:iterator>
						<!-- 分页展示功能块  -->
						...
						<div class="feiye-nav">
							<ul>
								<c:choose>
									...
									<c:otherwise>
										<li>
											<a class="a-cont" href="userSelectAction_showSeclectByPage?currentPage=1">«首页</a>
										</li>
										<li>
											<a class="a-cont" href="userSelectAction_showSeclectByPage?currentPage=${pageBean.currentPage-1}">«上一页</a>
										</li>
									</c:otherwise>
								</c:choose>
								...
								<c:forEach begin="${pageBean.forEachBegin }" end="${pageBean.forEachEnd}" varStatus="tp">
									<c:choose>
										...
										<c:otherwise>
											<li>
												<a class="a-cont" href="userSelectAction_showSeclectByPage?currentPage=${tp.index}">${tp.index}</a>
											</li>
										</c:otherwise>
									</c:choose>
								</c:forEach>
								...
								<c:choose>
									<c:when test="${pageBean.currentPage!=pageBean.totalPage}">
										<li>
											<a class="a-cont" href="userSelectAction_showSeclectByPage?currentPage=${pageBean.currentPage+1}">下一页»</a>
										</li>
										<li>
											<a class="a-cont" href="userSelectAction_showSeclectByPage?currentPage=${pageBean.totalPage}">尾页»</a>
										</li>
									</c:when>
									<c:otherwise>
										<li>
